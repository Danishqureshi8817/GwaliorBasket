...
export default function LoginDialog(props) {
 ...
 async function handleOTP() {
    if(testOtp==otp)
    {  
      var result = await posttData("userinterface/add_new_user",{mobileno:number})
      ...
      if (result.status==0){
        ...
        if(result.status==2){
          ...
          var result_address=await posttData('userinterface/check_user_address',{mobileno:number})
          console.log("address exit",result_address)
  ...
  //  var result=await postData('smsapi/sendotp',{mobileno:number,otp:gotp}) for sms
   alert(gotp)
   setTestOtp(gotp)
  }   
  const handleClick = () => {
