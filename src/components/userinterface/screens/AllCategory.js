...
import { getData, posttData, ServerURL } from "../../services/ServerServices";
...
const AllCategory = (props) => {
  ...
  async function getProducts() {
    let result = await posttData("userinterface/fetch_products", { categoryid: categoryId });
    setProducts(result.data);
  }

  async function getProductList() {
    let result = await posttData("userinterface/fetch_all_productlist_by_product",{productid:productId});
      ...
      return (
        <Grid key={i} item xs={6} md={2.8} style={{ height: matches ? 240 : 300 }}>
          <Card onClick={()=>handleExploreProduct(item)} sx={{ border: "1px solid #ced4d6", height: "90%", width: "80%", boxShadow: "none" }}>
            <div style={{ height: "45%", marginTop: 7 }}>
